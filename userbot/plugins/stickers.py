...
CUSTOM_STICKER_NAME =str(CUSTOM_STICKER_PACK_NAME) if CUSTOM_STICKER_PACK_NAME else "My Boss HunterGang Volume Pack One"
...
async def _(event):
    ...
    #packname = f"HunterGang PACK"
    #packshortname = f"HunterGang_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1263617196:
        packname = f"@HunterGangPacks 🎭"
        packshortname = "HunterGang Pack"
    else:
        packname = f"{user.first_name}'s HunterGang Vol.{pack}"
        packshortname = f"HunterGang_{userid}_Pack"
    await event.edit("`Is It Legal ? Oh Yes it is !! \nLook That Way ! Let me Kang This ¯\_(ツ)_/¯`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@HunterGang.png"
    ...
    if is_a_s:
        ...
        if userid == 813878981:
            packname = f"HunterGangKa Pack"
            packshortname = "HunterGangisgreat"
        else:
            packname = f"{user.first_name}'s HunterGang Animated Vol.{pack}"
            packshortname = f"HunterGang_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 948408212:
                       # packname = f"{user.first_name}'s HunterGang Vol.{pack}"
                       # packshortname = "Vol._{pack}_HunterGang_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_HunterGang{userid}"
                        #packshortname = f"Vol._{pack}_HunterGang_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
